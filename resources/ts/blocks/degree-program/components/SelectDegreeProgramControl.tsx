...
export default function SelectDegreeProgramControl( {
	...
	const [ formTokenFieldValue, setFormTokenFieldValue ] =
		useState< string >( '' );
	...
	>(
		( select ) =>
			select( degreeProgramStore.name ).getDegreeProgram( value, lang ),
		[ value, lang ]
	);

	const searchedDegreePrograms: Array< DegreeProgram > | null | undefined =
		useSelect< MapSelect< DegreeProgramStore > >(
			( select ) =>
	...
	const findDegreeProgramByToken = useCallback<
		( token: string ) => DegreeProgram | undefined
	>(
		( token ) => {
			const availableDegreePrograms = [
				...( searchedDegreePrograms ?? [] ),
			];
			if ( degreeProgram ) {
			...
			return find(
				...
				( item ) => degreeProgramToToken( item ) === token
			);
		},
			...
			<div tabIndex={ -1 } role="listbox">
				<FormTokenField
					...
					__experimentalValidateInput={ ( token ) =>
						!! findDegreeProgramByToken( token )
					}
